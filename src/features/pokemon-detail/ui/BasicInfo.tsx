...
import { describeGender } from '../domain/describeGender'
...
const BasicInfo = ({ height, weight, gender_rate, capture_rate, hatch_counter, base_experience, growth_rate, egg_groups }: Props) => {
	const gender = describeGender(gender_rate)
							...
							gender.kind === 'ratio' ? (
								<div className='flex gap-2'>
									<div className='flex items-center'>
										...
										<span className='text-xs text-[#7B7B7B] font-bold'>{gender.malePercent}%</span>
									</div>
									<div className='flex items-center'>
										<MdFemale className='text-lg' color='rgb(219,39,119)' />
										<span className='text-xs text-[#7B7B7B] font-bold'>{gender.femalePercent}%</span>
									</div>
								</div>
							) : (
								<div>
