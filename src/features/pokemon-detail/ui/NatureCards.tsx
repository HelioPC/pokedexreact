...
import { Pokemon, Type } from '../../../data/pokeapi'
...
const NatureCards = ({ pokemon, types }: Props) => {
	return (
		<AnimatedCard
			...
			classProps02='h-auto min-h-[50vh] sm:w-2/3 w-[80%] py-4 absolute bottom-[0px] left-0 rounded-lg shadow-xl border-2 border-solid'
...
const NatureCompactCard = ({ pokemon, types }: Props) => {
	return (
		<div className='w-full h-full flex flex-row flex-wrap justify-center p-2 gap-10'>
			...
			{types[0].damage_relations.double_damage_from.length > 0 ?
				(
					<div className='flex flex-col items-center p-2'>
			...
			{types[0].damage_relations.double_damage_to.length > 0 ?
				(
					<div className='flex flex-col items-center p-2'>
